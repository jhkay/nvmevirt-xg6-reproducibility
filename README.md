# NVMeVirt XG6 Reproducibility Appendix

본 저장소는 논문 「NVMe 에뮬레이터 NVMeVirt의 SSD 성능 재현 한계와 원인 분석」에 대한 부록 자료를 제공하기 위해 구성되었습니다.

벤치마크 로그, 실험 환경 제어 명령어, 반복성 검증 데이터, fio 설정 등을 공개 GitHub 링크를 통해 참조할 수 있도록, 해당 부록 자료는 원고에서 분리하여 본 저장소에 수록하였습니다.

## Appendix

- [A.1 순차쓰기 워크로드의 벤치마크 로그](appendix.md#a1)
- [A.2 실험 환경 통제를 위한 설정 명령](appendix.md#a2)
- [A.3 통제 항목 변경에 따른 상대 성능 변화](appendix.md#a3)
- [A.4 반복 측정 변동성 요약](appendix.md#a4)
- [A.5 측정 단계에 사용한 fio job 파일](appendix.md#a5)

## Repository Contents

- [`appendix.md`](appendix.md): 전체 부록 자료

## Related Links

- [NVMeVirt 공식 저장소](https://github.com/snu-csl/nvmevirt)
- [NVMeVirt 논문 (FAST 2023)](https://www.usenix.org/conference/fast23/presentation/kim-sang-hoon)
